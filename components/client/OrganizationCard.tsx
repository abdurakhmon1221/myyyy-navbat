    ...
    return (
        <div key={org.id} className="glass rounded-[2.5rem] p-6 shadow-sm border border-gray-100 group transition-all hover:shadow-md animate-slide-up relative overflow-hidden bg-[var(--bg-card)]">
            <button
                ...
                className={`absolute top-6 right-6 p-2 rounded-xl transition-all ${isFav ? 'bg-rose-50 text-rose-500' : 'bg-gray-50 text-gray-400 opacity-0 group-hover:opacity-100'}`}
            >
                <Heart size={18} fill={isFav ? 'currentColor' : 'none'} />
            </button>

            <div className="flex items-center gap-5 mb-5">
                <div className="w-16 h-16 bg-emerald-50 dark:bg-emerald-900/30 rounded-[1.5rem] flex items-center justify-center text-emerald-600 transition-transform group-hover:scale-110">
            ...
            <div className="flex gap-2">
                <button onClick={() => onJoinQueue(org)} className="flex-[2] py-4 rounded-2xl bg-emerald-500 text-white font-black text-xs uppercase shadow-xl hover:bg-emerald-600 active:scale-95 transition-all">{t('join_queue')}</button>
